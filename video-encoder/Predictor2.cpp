...
void Predictor::spatialPredict(int mode) {
	...
	for(int i = 0; i < nFrames; i++) {
		cap.read(frame);
		encodeIntraFramerame(frame, mode, i==nFrames-1);
	}
	//Mat img = imread(file->c_str(), 1);
}

void Predictor::encodeIntraFramerame(Mat frame, int mode, int isLastFrame) {
